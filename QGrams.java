public class QGrams	{
	...
	double q_grams_distance ( String str1, String str2, int qNumber)	{
		...
		stringConstruction();	
		calculateDistance();	//0 if s1=s2, 1 otherwise
	...
	void calculateDistance()	{
		...
		//loop to compare substrings of s1 & s2 of length q	
		for (int i = 0; i <= length1 - q; i++)	{
			substringCount++;
			for (int j = 0; j <= length2 - q; j++)	{
				if (i == 0)	substringCount++;
				if (s1.substring(i, i + q).equals(s2.substring(j, j + q)))	overlapSubstringCount++;
			}
		}
		//calculating normalized distance
		distance = 1.0 - ((double)overlapSubstringCount/(substringCount - overlapSubstringCount));	
	}
}
